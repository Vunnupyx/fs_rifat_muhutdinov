...
import {GetAnimalAction, UpdateAnimalAction} from "../store/data.actions";
import {selectAnimal} from "../store/data.selectors";
...
export class AnimalEditComponent implements OnInit {
  ...
  constructor(private store$: Store<Animal>, private _dataService: DataService, private _route: ActivatedRoute) {
  ...
  ngOnInit() {
    this._route.paramMap.pipe(
      ...
      this.store$.dispatch(new GetAnimalAction({id}))
      this.store$.select(selectAnimal).subscribe(animal => {
        this._animal = Object.assign({}, animal)
        this._initForm()
      });
  ...
  public _onSubmit() {
    if (this._reactiveForm.valid) {
      ...
      const animal: Animal = Object.assign({}, this._animal)
      this.store$.dispatch(new UpdateAnimalAction({animal}))
    }
  }
